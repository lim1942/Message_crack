...
def collect_accounts():
    for account in search_all('app.cloopen.com', max_page=10):
        try:
...
if __name__ == '__main__':
    ...
    while True:
        time.sleep(1)
        try:
            with open('mobile.txt',encoding='utf-8') as f:
                phone = f.read().strip()
        ...
        try:
            start = time.time()
            try:
                gevent.joinall([gevent.spawn(run, account, phone) for account in accounts])
